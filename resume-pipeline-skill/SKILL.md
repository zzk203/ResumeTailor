...
对每个 JD，读取以下 4 个 jinja2 模板（位于 `resume-matcher-skill/src/prompt_templates/`），**agent 直接生成**每项内容：

| 类型 | 模板文件 | 说明 |
|------|----------|------|
| first-msg | `first_message.jinja2` | 投递第一句话（支持 --channel im\|email） |
| interview-intro | `interview_intro.jinja2` | 面试自我介绍（支持 --interview-duration 1min\|3min） |
| skill-gap | `skill_gap.jinja2` | 能力补充建议（支持 --skill-focus gap\|strength\|balanced） |
| project-recommend | `project_recommend.jinja2` | 练习项目推荐（支持 --project-level beginner\|intermediate\|advanced） |

**agent 注意**：你仍然是 LLM，直接读取模板 → 理解 → 生成内容。不要调用 Python。

扩展材料输出到 `resume/{姓名}-{岗位}-{公司}-extras.md`，格式：
