---
name: resume-matcher
description: 根据简历模板、个人经历和岗位描述自动生成定制简历。用户提供模板（含 fixed/fill 标签）、个人信息文件、JD 文件，Agent 自动匹配并生成个性化简历。
---
...
### 固定标签（不被修改）
```
<!-- fixed:字段名 --> 内容 <!-- /fixed:字段名 -->
```
Agent 处理时**完整保留**标签及其内部内容。
...
- `python src/tools.py assemble <template_file> <fill_mapping_json> <user_name> <job_title> <company> [output_dir]` — 组装简历
