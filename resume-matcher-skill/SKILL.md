...
- 扩展材料 `.md` 文件（可选），文件名：`{姓名}-{岗位名称}-{公司名}-extras.md`
...
resume-matcher 提供 4 种扩展材料生成，通过 `generate-extras` 子命令调用：
...
# 批量组装（jd_dir 下 .md + .json 配对）
python -m resume-matcher-skill.src.tools batch-assemble <template> <personal> <jd_dir> [--output-dir ./resume] [--clean-json]
